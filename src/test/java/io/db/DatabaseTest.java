...
public class DatabaseTest {
    ...
    public void testLoadStore() {
        ...
        List<Item> items = Database.loadStore().items();
    ...
    public void testLoadAndUpdateItem() {
        ...
        Item item = Database.loadStore().items().get(0);
        item.update("item2", 200, 2);
        //db.updateItem(item);
        verify(Database.engine).updateItem(101, "item2", 200, 2);
    }

    @Test
    public void testInsertAndUpdateItem() {
        Item item = new Item("item1", 100, 1);
        DbEngine engine = mock(Database.class);
        when(Database.engine.insertItem("item1", 100, 1)).thenReturn(101);
        Database.loadStore();
        //db.addItem(item);
        item.update("item2", 200, 2);
        //db.updateItem(item);
